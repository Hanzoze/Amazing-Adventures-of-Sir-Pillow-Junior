...
func _ready() -> void:
	...
	# Используем call_deferred — сцена ещё не готова в _ready
...
func start_next_wave() -> void:
	current_wave += 1
	print("--- НАЧАЛАСЬ ВОЛНА НОМЕР: ", current_wave, " ---")
	var enemies_to_spawn = current_wave
	active_enemies_count = enemies_to_spawn
...
func spawn_enemy() -> void:
	if not enemy_scene:
		print("Ошибка: сцена врага не указана!")
		return
	...
	if spawn_points.size() > 0:
		...
		# Разброс ±3 единицы по X и Z чтобы демоны не стояли в одной точке
...
func _on_enemy_killed() -> void:
	...
	print(">>> enemies_killed = ", Global.enemies_killed, " | active = ", active_enemies_count)
	...
	print("Враг убит! Осталось: ", active_enemies_count)
	
	if active_enemies_count <= 0:
		active_enemies_count = 0
		print("Волна пройдена! Следующая через 3 сек...")
